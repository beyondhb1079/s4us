...
3. **Handling Rejections:** If your push is rejected (`non-fast-forward`), it means another agent beat you to the remote. Do not panic. Simply run `git pull origin <branch-name>`, resolve any standard merge conflicts in the code or `.TODO.md` file, commit the resolution, and run a standard `git push`.
   - **Post-Merge Verification:** After successfully pulling remote changes into your Epic branch, immediately run a quick local build (e.g., `yarn build` at the root for Frontend/UI, or `cd functions && yarn build` for the Data Shield)) to ensure the merge didn't introduce a silent logic break before you push.
