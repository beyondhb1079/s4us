...
**Cost & Rate Limits Assessment:**

- _Gemini 3.0 Flash limits:_ 15 RPM on the free tier. The queue processing _must_ stagger HTTP requests using `setTimeout` or strict Cloud Task dispatch configurations to guarantee we never exceed 15 RPM.
- _Firestore:_ Minimal impact. Writes are tiny.
